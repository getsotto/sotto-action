...
Check the [Security model](#security-model) for supported platforms and confirm that the runner's operating system and architecture are supported.

When reporting a persistent installation problem, include the relevant error message and logs, but remove tokens, secrets, and other sensitive information before sharing them.

For unresolved problems, open an issue in the [Sotto Action repository](https://github.com/getsotto/sotto-action/issues).
